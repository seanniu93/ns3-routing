...
def get_min_cost(mymap, result):
    min = 999999
    for node in mymap:
        if result[node] == False:
            cost = mymap[node][0]
            if cost < min:
                min = cost
                winner = node
...
result = dict([(node,False) for node in neighbor_cost_map])
...
for i in range(N-1):
    ...
    current = get_min_cost(least_costs_and_nexthop, result)
    print "Current node:",current
    print "Least costs table:",least_costs_and_nexthop
    print "result:", result
